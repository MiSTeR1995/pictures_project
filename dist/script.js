  ...
  txtInputs.forEach(function (input) {
    input.addEventListener('keypress', function (e) {
      ...
      if (e.key.match(/[^а-яё 0-9]/gi)) {
        // ^ - в регулярке означает начало ввода, ё нужно указать отдельно
...
var forms = function forms() {
  ...
  upload.forEach(function (item) {
    item.addEventListener('input', function () {
      ...
      var dots;
      var arr = item.files[0].name.split('.'); // разбить строку имени по точке, если в названии > 5 символов, то в переменную dots запишем ...

      arr[0].length > 6 ? dots = '...' : dots = '.'; // теперь формируем короткое название файла

      var name = arr[0].substring(0, 6) + dots + arr[1]; // взглянув на верстку, нам нужно получить предыдущий соседний элемент
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      ...
      statusMessage.classList.add('status'); // так как нужн скрыть форму, то блок поместить нужно в родителя формы

      item.parentNode.appendChild(statusMessage); // скрываем форму с анимацией

      item.classList.add('animated', 'fadeOutUp');
      setTimeout(function () {
        item.style.display = 'none';
      }, 400); // Помещаем изображение спиннера на страницу

      var statusImg = document.createElement('img');
      statusImg.setAttribute('src', message.spinner);
      statusImg.classList.add('animated', 'fadeInUp');
      statusMessage.appendChild(statusImg); // текстовой сообщение

      var textMessage = document.createElement('div');
      textMessage.textContent = message.loading;
      statusMessage.appendChild(textMessage); // сбор данных в форме через FormData

      var formData = new FormData(item);
      var api; // переменная для формирования динамического пути
      ...
      Object(_services_requests__WEBPACK_IMPORTED_MODULE_6__["postData"])(api, formData).then(function (res) {
        ...
        textMessage.textContent = message.succes; // setTimeout(() => {
...
var mask = function mask(selector) {
  var setCursorPosition = function setCursorPosition(pos, elem) {
    ...
    if (elem.setSelectionRange) {
      // для новых у которых есть этот метод
      elem.setSelectionRange(pos, pos); // если в нач и конеч позицию передать одно и тоже, то курсор уст. в опр. позицию
    } else if (elem.createTextRange) {
      // для старых браузеров с таким методом
      var range = elem.createTextRange(); // диапазон который нужно выделить на основе этого метода
  ...
  function createMask(event) {
    var matrix = '+7 (___) ___ __ __'; // матрица для создания маски

    var i = 0; // статичное значение работает на основе матрицы

    var def = matrix.replace(/\D/g, ''); // заменяем все не цифры через регулярное выражение
    // динамичное значение, работает на основе введенных пользователем данных

    var val = this.value.replace(/\D/g, ''); // обращение к контексту и использование его value
  ...
  inputs.forEach(function (input) {
    ...
    input.addEventListener('input', createMask); // тоже самое для фокуса и блюра
...
var showMoreStyles = function showMoreStyles(trigger, wrapper) {
  ...
  btn.addEventListener('click', function () {
    Object(_services_requests__WEBPACK_IMPORTED_MODULE_4__["getResource"])('http://localhost:3000/styles') // можно обратится к assets/db.json. Тогда ниже нужно будет обращаться к res.style
    .then(function (res) {
...
var slider = function slider(slides, direction, prev, next) {
  var slideIndex = 1; // текущий слайд

  var paused = false; // для остановки переключения слайдов
  ...
  try {
    ...
    prevBtn.addEventListener('click', function () {
      plusSlides(-1); // настройка анимации
  ...
  activateAnimation(); // при наведении на родителя слайда удаляется интервал
